...
export function addUser(
  ...
  return function (dispatch) {
    db.transaction((tx) => {
      tx.executeSql(
        'INSERT INTO users (studentId,nameSurname,email,phone,faculty,department) VALUES (?,?,?,?,?,?)',
        [studentId, nameSurname, email, phone, faculty, department],
        (tx, results) => {
...
export function updateUser(
  ...
  return function () {
    db.transaction((tx) => {
      tx.executeSql(
        'UPDATE users SET studentId=?, nameSurname=?, email=?, phone=?, faculty=?, department=? WHERE id=?',
        [studentId, nameSurname, email, phone, faculty, department, id],
        (tx, results) => {
          if (results.rowsAffected > 0) {
